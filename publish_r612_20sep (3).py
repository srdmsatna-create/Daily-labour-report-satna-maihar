...
Usage: py -3 publish_r612_20sep.py VBGRAMG_R612_Satna_Maihar_20-09-2026.xlsx C:/path/to/Daily-labour-report-satna-maihar --nrega-baseline VBG_RAMG_17.08.2026_FINAL_CORRECTED_REPAIRED.xlsx
...
<div class="box"><p>MGNREGA के 13.09.2026 fixed baseline कॉलम मूल फ़ाइल से सुरक्षित रखे गए हैं। NREGA Current FY Mandays = 01 अप्रैल–30 जून; NREGA Total Mandays = कार्य प्रारंभ–30 जून। जिन Work Code का स्रोत से मिलान नहीं हुआ, उनका Mandays खाली है।</p>
<input id="q" placeholder="कार्य कोड, जनपद, उपयंत्री या श्रेणी खोजें" aria-label="खोज"><p id="summary"></p>
<div class="scroll"><table><thead><tr><th>जिला</th><th>जनपद</th><th>GP</th><th>उपयंत्री</th><th>क्लस्टर</th><th>कार्य कोड</th><th>कार्य</th><th>श्रेणी</th><th>स्थिति</th><th>स्वीकृत ₹</th><th>बुक्ड ₹</th><th>NREGA Mandays 01 Apr–30 Jun</th><th>NREGA Mandays कार्य प्रारंभ–30 Jun</th><th>MGNREGA स्रोत मिलान</th><th>VBGRAMG Portal Total Mandays</th></tr></thead><tbody id="rows"></tbody></table></div>
...
def main():
    ...
    parser.add_argument("--nrega-baseline", required=True, type=Path,
                        help="17 Aug dual-source workbook with VBG RAMG Main / NREGA Total Mandays")
    parser.add_argument("--prepare-only", action="store_true", help="Generate files without git commit/push")
    args = parser.parse_args()
    repo = args.repo.resolve()
    if not args.workbook.is_file() or not args.nrega_baseline.is_file() or not repo.is_dir():
        parser.error("Excel file, MGNREGA baseline, or repository directory not found")
    ...
    older = load_workbook(args.nrega_baseline, read_only=True, data_only=True)
    if "VBG RAMG Main" not in older:
        sys.exit("MGNREGA baseline sheet VBG RAMG Main is missing")
    source_rows = older["VBG RAMG Main"].values
    source_header = list(next(source_rows))
    if any(k not in source_header for k in ("Work Code", "NREGA Total Mandays", "NREGA Current FY Mandays")):
        sys.exit("MGNREGA baseline Work Code / Mandays columns missing")
    ni = source_header.index("NREGA Total Mandays")
    ci = source_header.index("NREGA Current FY Mandays")
    wi = source_header.index("Work Code")
    nrega = {}
    for source_row in source_rows:
        work_code = source_row[wi]
        if work_code:
            if str(work_code) in nrega:
                sys.exit("Duplicate MGNREGA baseline Work Code: " + str(work_code))
            nrega[str(work_code)] = (source_row[ci], source_row[ni])
    ...
    for name in ("NREGA Mandays 01 Apr to 30 Jun (17 Aug fixed source)",
                 "NREGA Mandays Work Start to 30 Jun (17 Aug fixed source)",
                 "MGNREGA Work Code Source Match"):
        columns.append(name)
        index[name] = len(columns) - 1
    ...
    for row in source:
        ...
        # A missing source match is unknown, never a reported zero.
        nrega_value = nrega.get(str(code))
        groups[status].append([plain(value) for value in row] +
                              ([plain(value) for value in nrega_value] if nrega_value is not None else ["", ""]) +
                              ["मिला" if nrega_value is not None else "स्रोत में नहीं मिला"])
    ...
    unmatched = [row for row in groups["Ongoing"] if row[index["MGNREGA Work Code Source Match"]] == "स्रोत में नहीं मिला"]
    expected_unmatched = {"PMAY-G": 223, "IAY Houses": 3,
                          "Watershed Related Works": 6, "Water conservation & recharge": 3}
    actual_unmatched = dict(Counter(row[index["Final Category"]] for row in unmatched))
    if actual_unmatched != expected_unmatched:
        sys.exit(f"Unmatched source/category drift: {actual_unmatched}. No files changed.")
    ...
    selected = ["District", "Janpad", "Panchayat", "Sub Engineer / Upyantri (13 Sep mapping)",
                ...
                "Work Status", "Sanction Total Rs", "Booked Total Since Inception Rs",
                "NREGA Mandays 01 Apr to 30 Jun (17 Aug fixed source)",
                "NREGA Mandays Work Start to 30 Jun (17 Aug fixed source)",
                "MGNREGA Work Code Source Match", "Total Mandays (portal)"]
    outputs = []
    ...
    matched = sum(str(row[index["Work Code"]]) in nrega for row in groups["Ongoing"])
    print(f"MGNREGA work-start-to-30-Jun matched: {matched:,} Ongoing; source-unmatched: {15618-matched:,} (blank, not zero).")
    apr_jun = sum(int(float(row[index["NREGA Mandays 01 Apr to 30 Jun (17 Aug fixed source)"]] or 0))
                  for row in groups["Ongoing"])
    print(f"Matched Ongoing NREGA 01-Apr-to-30-Jun mandays: {apr_jun:,}.")
