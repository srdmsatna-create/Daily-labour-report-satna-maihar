...
title SRDM SATNA - Fixed Root Deploy V11
color 0A
...
echo Source     : %SOURCE%
echo Git root   : %REPO%
echo Replacing old 301 KB root page with latest full dashboard...
...
cd /d "%REPO%"
for %%F in (index.html auto-data.js auto-status.js yuktdhara-data.js yuktdhara-subengineer.js muster-emb-data.js) do echo %%F & dir /-C "%%F" | find "%%F"
...
git commit -m "Replace old root page with full Yuktdhara dashboard V11"
...
echo.
echo ============================================================
echo SUCCESS - Full dashboard pushed from actual Git root.
echo Confirm index.html size above is about 10,807,000 bytes.
...
:nosource
color 0C
echo ERROR: final\index.html is missing from extracted V11 package.
pause
exit /b 1

:nogit
color 0C
echo ERROR: Actual Git root was not found at:
echo %REPO%
pause
exit /b 2
...
:copyfail
color 0C
echo ERROR: File copy to actual Git root failed.
pause
exit /b 3
