...
def parse(path):
    ...
    rep=wb['RepDay']; title=c(val(rep,1,1)); rows=[]; gpmap={}
    ...
    return {'title':title,'rows':rows,'official':official,'workmix':list(wm.values())}

path,source=obtain();data=parse(path)
data['meta']={'mode':'auto','status':'ok','updatedAt':datetime.now(timezone.utc).isoformat(),'source':source,'rowCount':len(data['rows'])}
