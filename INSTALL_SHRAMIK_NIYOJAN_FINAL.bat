...
for /d %%D in ("%USERPROFILE%\Daily-labour-report-satna-maihar*") do if exist "%%~fD\index.html" set "REPO=%%~fD"
if exist "%REPO%\index.html" goto REPO_FOUND
for /d %%D in ("%USERPROFILE%\Downloads\Daily-labour-report-satna-maihar*") do if exist "%%~fD\index.html" set "REPO=%%~fD"
if exist "%REPO%\index.html" goto REPO_FOUND
...
if not exist "%REPO%\index.html" (
  echo ERROR: Is folder me index.html nahi mila:
  echo %REPO%
  pause
  exit /b 1
)
