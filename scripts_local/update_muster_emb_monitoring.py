...
def fetch():
    ...
    # The portal omits GPs where no Muster Roll has been issued. Add those
    # mapped GPs with zero values so the dashboard always shows all 695 GPs.
    present={(r["block"],r["panchayat"]) for r in gp_rows}
    for (block,gp),mp in gpmap.items():
        if block in BLOCKS and (block,gp) not in present:
            z={"block":block,"panchayat":gp,"engineer":mp["engineer"],"cluster":mp["cluster"]}
            z.update({k:0 for k in keys});gp_rows.append(z)
    gp_rows.sort(key=lambda r:(r["block"],r["panchayat"]))
    if len(gp_rows)<695: raise RuntimeError(f"Expected all 695 Satna GPs, got only {len(gp_rows)} rows")
...
#musterEmbLauncher .srdm-app-icon{background:#e65100!important;color:#fff!important}.muster-emb-section{margin:18px 0;background:#fff;border:2px solid #e65100;border-radius:20px;padding:12px;box-shadow:0 10px 28px #e6510022}.muster-emb-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:10px}.muster-emb-head h2{margin:0;color:#b33d00}.muster-emb-pill{background:#fff0e3;color:#a43700;border-radius:999px;padding:6px 10px;font-weight:900;font-size:11px}.muster-emb-wrap{overflow:auto}.muster-emb-table{width:100%;border-collapse:collapse;min-width:1080px;table-layout:fixed}.muster-emb-table th{background:#b33d00;color:#fff;padding:7px 3px;border:1px solid #d26932;font-size:10px;line-height:1.15;white-space:normal}.muster-emb-table td{padding:6px 4px;border:1px solid #f0d2c2;text-align:right;font-size:11px;font-weight:700;overflow-wrap:anywhere}.muster-emb-table td:nth-child(2),.muster-emb-table td:nth-child(3),.muster-emb-table td:nth-child(11){text-align:left}.muster-emb-table th:nth-child(1){width:34px}.muster-emb-table th:nth-child(2){width:92px}.muster-emb-table th:nth-child(3){width:145px}.muster-emb-table th:nth-child(4),.muster-emb-table th:nth-child(5),.muster-emb-table th:nth-child(7){width:64px}.muster-emb-table th:nth-child(6){width:82px}.muster-emb-table th:nth-child(8),.muster-emb-table th:nth-child(9){width:92px}.muster-emb-table th:nth-child(10){width:105px}.muster-emb-table th:nth-child(11){width:135px}.muster-emb-table tbody tr:nth-child(even){background:#fff8f3}.muster-emb-table .total-row{background:#ffe6d5!important;color:#8b2e00;font-weight:950}.muster-emb-source{text-align:right;margin-top:8px;font-size:11px;color:#667085}
/* V13 readable premium compact theme */
#musterEmbLauncher{--app-accent:#0f4c81!important;--app-soft:#eaf4ff!important;border-color:#0f4c81!important;background:linear-gradient(135deg,#eaf4ff,#ffffff 72%)!important}
#musterEmbLauncher .srdm-app-icon{background:linear-gradient(135deg,#0f4c81,#168aad)!important}.muster-emb-section{border-color:#168aad;background:linear-gradient(180deg,#f7fbff 0,#fff 150px);box-shadow:0 12px 32px #0f4c8120}.muster-emb-head h2{color:#0b3b66}.muster-emb-pill{background:#dff3ff;color:#075985;border:1px solid #7dd3fc;font-size:12px}
.muster-emb-table{width:1180px!important;min-width:1180px!important;max-width:none!important;margin:0!important;background:#fff}.muster-emb-table th{background:linear-gradient(180deg,#0f4c81,#0b3b66)!important;border-color:#3979aa!important;font-size:12px!important;line-height:1.25!important;padding:9px 4px!important}.muster-emb-table td{font-size:13px!important;line-height:1.25!important;padding:7px 5px!important;border-color:#cbddec!important;color:#102a43}.muster-emb-table tbody tr:nth-child(even){background:#f1f7fc!important}.muster-emb-table tbody tr:hover{background:#dff3ff!important}.muster-emb-table .total-row{background:#d8efff!important;color:#073b66!important}
.muster-emb-table th:nth-child(1){width:38px}.muster-emb-table th:nth-child(2){width:105px}.muster-emb-table th:nth-child(3){width:155px}.muster-emb-table th:nth-child(4),.muster-emb-table th:nth-child(5),.muster-emb-table th:nth-child(7){width:72px}.muster-emb-table th:nth-child(6){width:92px}.muster-emb-table th:nth-child(8),.muster-emb-table th:nth-child(9){width:102px}.muster-emb-table th:nth-child(10){width:118px}.muster-emb-table th:nth-child(11){width:150px}
#musterZeroAlert{border-color:#f59e0b!important;background:linear-gradient(135deg,#fff8e7,#fff1f2)!important;color:#8a3414!important;box-shadow:0 5px 15px #f59e0b20}#musterZeroAlert span{background:#fff!important;border:1px solid #fdba74!important;color:#9a3412!important}
#musterBlockFilter,#musterEngineerFilter{border:1px solid #168aad!important;background:#fff;color:#0b3b66;font-size:13px;font-weight:800}
...
def install():
    ...
    alert_box='''<div id="musterZeroAlert" style="display:none;margin:8px 0 12px;padding:10px 12px;border:2px solid #dc2626;border-radius:12px;background:#fff1f2;color:#991b1b;font-size:12px;line-height:1.5"></div>'''
    safe_section=safe_section.replace('<div class="muster-emb-wrap">',alert_box+'<div class="muster-emb-wrap">',1)
    alert_script='''<script>(function(){var d=window.MUSTER_EMB_REPORT,e=document.getElementById('musterZeroAlert');if(!d||!e||!Array.isArray(d.gpRows))return;var z=d.gpRows.filter(function(r){return Number(r.issued||0)===0});if(!z.length)return;e.style.display='block';e.innerHTML='<b>⚠ Muster Roll जारी नहीं — '+z.length+' Gram Panchayat Alert</b><br>'+z.map(function(r){return '<span style="display:inline-block;margin:4px 7px 0 0;padding:3px 7px;border-radius:999px;background:#fee2e2"><b>'+r.panchayat+'</b> • '+r.block+' • '+(r.engineer||'Unmapped')+'</span>'}).join('');})();</script>'''
    safe_section=safe_section.replace('<!-- SRDM_MUSTER_EMB_SECTION_END -->',alert_script+'<!-- SRDM_MUSTER_EMB_SECTION_END -->')
