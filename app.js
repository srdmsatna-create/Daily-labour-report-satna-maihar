...
function updateAutoStatus(){const el=$('autoStatus');if(!el)return;const mode=autoMeta?.mode||'sample';const when=autoMeta?.updatedAt?new Date(autoMeta.updatedAt).toLocaleString('hi-IN'):'—';const source=autoMeta?.source||'Bundled sample data';const ok=autoMeta?.status==='ok';const sd=autoMeta?.sourceDates||{};const ds=[sd.Sheet1?`Sheet1: ${sd.Sheet1}`:'',sd.RepDay?`RepDay: ${sd.RepDay}`:''].filter(Boolean).join(' • ');el.innerHTML=`<span class="status-pill ${ok?'ok':'sample'}">${ok?'AUTO DATA':'SAMPLE / MANUAL'}</span><b>${ok?'Last auto update':'Current mode'}:</b> ${ok?esc(when):esc(mode)} <span class="status-source">${esc(source)}</span>${ds?` <span class="status-source">| Data date: ${esc(ds)}</span>`:''}`;}
...
function renderOngoingDetails(){
 ...
 lastExport=data;$('viewTitle').textContent='Ongoing Works — Janpad-wise Work Sheet';$('viewMeta').innerHTML=`${fmt(data.length)} ongoing works • dynamic_work_details work-level data • <a href="data/Ongoing_Works_dynamic_work_details_Janpad_Wise_23-08-2026.csv" download>Download Full CSV</a> • ${todayDate()}`;
...
function render(){const list=filteredRows();$('reportDate').textContent=todayDate();kpis(list);renderAlerts(list);$('reportTable').className='report-table '+(view==='official'?'official-grid':view==='engineer'?'engineer-grid':view==='district'?'district-grid':view==='priority'?'priority-grid':view==='dysfunctional'?'dysfunctional-grid':view==='category'?'category-grid':view==='expbucket'?'expbucket-grid':view==='ongoingall'?'ongoing-summary-grid':view==='ongoingdetails'?'ongoing-detail-grid':'gp-grid');if(view==='official')return renderOfficial();if(view==='ongoingall')return renderOngoingAllSummary();if(view==='ongoingdetails')return renderOngoingDetails();if(view==='category')return renderCategoryBuckets();if(view==='expbucket')return renderExpBuckets();if(view==='priority')return renderPriorityAlerts(list);if(view==='dysfunctional')return renderDysfunctionalBucket(list);if(view==='engineer'){const d=engineerData(list);return standardTable(d.sort((a,b)=>clean(a.janpad).localeCompare(clean(b.janpad),'hi')||clean(a.engineer).localeCompare(clean(b.engineer),'hi')),[['janpad','Janpad'],['engineer','Engineer / Upyantri'],['cluster','Cluster(s)'],['gps','GP',1],['gpsProgress','GP Progress',1],['labour','Labour',1],['ongoing','Ongoing Works',1],['worksMR','Works with MR',1],['mrs','Muster Rolls',1],['pmayOngoing','PMAY-G Ongoing',1],['ekOngoing','Ek Bagiya Ongoing',1],['currentFYActive','Current FY Active Works',1]],'Engineer-wise Daily Report — Official Sheet1 Reconciled')}if(view==='district'){if($('engineerFilter').value==='ALL'&&$('clusterFilter').value==='ALL'&&daily.length){const dd=dailyFiltered().map(r=>({...r,district:districtOf(r.janpad)}));const d=aggregate(dd,['district'],['totalGP','gpsProgress','labour','worksMR','mrs','noEkyc']);const om=aggregate(officialFiltered().map(r=>({...r,district:districtOf(r.janpad)})),['district'],['ongoingAll']);const ox=new Map(om.map(r=>[r.district,r.ongoingAll]));d.forEach(r=>r.ongoing=ox.get(r.district)||0);return standardTable(d,[['district','District'],['totalGP','Total GP',1],['gpsProgress','GP Progress',1],['labour','Labour',1],['ongoing','Ongoing Works',1],['worksMR','Works with MR',1],['mrs','Muster Rolls',1]],'Full District Report — Screen 2 Matched')}const d=aggregate(list.map(r=>({...r,district:districtOf(r.janpad)})),['district'],['gps','gpsProgress','ongoing','worksMR','labour','mrs','noEkyc']);return standardTable(d,[['district','District'],['gps','Total GP',1],['gpsProgress','GP Progress',1],['labour','Labour',1],['ongoing','Ongoing Works',1],['worksMR','Works with MR',1],['mrs','Muster Rolls',1]],'Full District Report')}return standardTable(list,[['janpad','Janpad'],['engineer','Engineer'],['cluster','Cluster'],['panchayat','Gram Panchayat'],['labour','Labour',1],['ongoing','Ongoing Works',1],['worksMR','Works with MR',1],['mrs','Muster Rolls',1]],'Gram Panchayat Detail')}
...
function parseWorkbook(wb){const a=sheetArray(wb,'RepDay');if(!a.length)throw new Error('RepDay sheet नहीं मिली।');reportTitle=clean(a?.[0]?.[0]);rows=[];for(let i=3;i<a.length;i++){const r=a[i];if(!clean(r[0])||!clean(r[1])||!clean(r[5]))continue;rows.push({janpad:normJanpad(r[0]),engineer:clean(r[1]),cluster:clean(r[2]),ongoing:num(r[3]),panchayat:clean(r[5]).toUpperCase(),gps:num(r[6]),gpsProgress:num(r[7]),labour:num(r[8]),worksMR:num(r[9]),noEkyc:num(r[10]),mrs:num(r[11])})}
 const s1=sheetArray(wb,'Sheet1');official=[];for(const ix of [3,4,5,7,8,9,10,11]){const r=s1[ix]||[];if(!clean(r[1]))continue;official.push({janpad:normJanpad(r[1]),totalGP:num(r[2]),musterGP:num(r[3]),dysfunctionalGP:num(r[4]),labourAll:num(r[5]),mrAll:num(r[6]),ongoingAll:num(r[7]),labourIndividual:num(r[9]),mrIndividual:num(r[10]),labourCommunity:num(r[11]),mrCommunity:num(r[12]),pmayOngoing:num(r[14]),pmayMR:num(r[15]),ekLabour:num(r[17]),ekOngoing:num(r[18]),ekMR:num(r[19])})}
 daily=[];let hix=s1.findIndex(r=>clean(r?.[2]).toLowerCase().includes('total no. of gram panchayats'));if(hix>=0){for(let i=hix+2;i<s1.length;i++){const r=s1[i]||[],j=clean(r[1]).toUpperCase();if(!j||j==='TOTAL')continue;if(!MAIHAR.has(j)&&!SATNA.has(j))continue;daily.push({janpad:j,totalGP:num(r[2]),gpsProgress:num(r[3]),labour:num(r[4]),worksMR:num(r[5]),noEkyc:num(r[6]),mrs:num(r[7])})}}
