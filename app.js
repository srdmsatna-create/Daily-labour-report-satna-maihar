...
function renderCategorySubEngineer(){
 ...
 const alert=$('reportDeficiencyAlert');if(alert){const examples=affected.slice(0,10).map(r=>`<span class="deficiency-location"><b>${clean(r.janpad)||'—'} • ${clean(r.engineer)||'—'}</b><small>${clean(r.category)||'Other'} — ${num(r.totalBooked)<=0?'Booked ₹ 0; ':''}${num(r.julyMandays)<=0?'01 Jul–Today Mandays 0; ':''}${r.expPct<25?'व्यय '+r.expPct.toFixed(1)+'%':''}</small></span>`).join('');alert.hidden=!affected.length;alert.innerHTML=affected.length?`<div class="deficiency-summary"><strong>⚠ कमी Alert — कहाँ सुधार आवश्यक है</strong><span>Booked राशि शून्य: <b>${fmt(zeroBooked.length)}</b></span><span>01 Jul–Today Mandays शून्य: <b>${fmt(zeroMandays.length)}</b></span><span>व्यय 25% से कम: <b>${fmt(lowExp.length)}</b></span></div><div class="deficiency-locations">${examples}${affected.length>10?`<span class="deficiency-more">और ${fmt(affected.length-10)} पंक्तियाँ तालिका में amber संकेत से चिन्हित हैं।</span>`:''}</div>`:''}
