...
function updateAutoStatus(){const el=$('autoStatus');if(!el)return;const mode=autoMeta?.mode||'sample';const when=autoMeta?.updatedAt?new Date(autoMeta.updatedAt).toLocaleString('hi-IN'):'';const source=autoMeta?.source||'Bundled sample data';const fetchStatus=window.AUTO_FETCH_STATUS||{};const ok=autoMeta?.status==='ok';const sd=autoMeta?.sourceDates||{};const ds=[sd.Sheet1?`Sheet1: ${sd.Sheet1}`:'',sd.RepDay?`RepDay: ${sd.RepDay}`:''].filter(Boolean).join(' • ');const changed=autoMeta?.dataChangedOnLastFetch;const changeMsg=changed===false?' | No new data on last check':changed===true?' | Fresh data changed':'';el.innerHTML=`<span class="status-pill ${ok?'ok':'sample'}">${ok?'AUTO DATA':'SAMPLE / MANUAL'}</span><b>${ok?'Last data change':'Current mode'}:</b> ${ok?esc(when):esc(mode)} <span class="status-source">${esc(source)}</span>${changeMsg?` <span class="status-source">${esc(changeMsg)}</span>`:''}${ds?` <span class="status-source">| Data date: ${esc(ds)}</span>`:''}${fetchStatus.finishedAt?` <span class="status-source">| Source checked: ${fetchStatus.ok?'✅':'⚠️'} ${esc(new Date(fetchStatus.finishedAt).toLocaleString('hi-IN'))}</span>`:''}`;}
