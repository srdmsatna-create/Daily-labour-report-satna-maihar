...
function rebuildCorrectedWorkData(){
 ...
 for(const r of ongoingDetails){
  const jan=clean(r.janpad),eng=clean(r.engineer)||'Unmapped',cl=clean(r.cluster)||'Unmapped',gp=clean(r.panchayat),cat=resolvedFinalCategory(r),san=num(r.sanction),book=num(r.booked),ep=san?book*100/san:0;
...
function ekBagiyaFiltered(){
  ...
  return ongoingDetails.filter(r=>{
    ...
    if((resolvedFinalCategory(r))!=='Ek Bagiya Maa Ke Naam' && (resolvedFinalCategory(r))!=='Ek Bagiya')return false;
...
function mandaysGenerationData(){
 ...
 for(const r of src){
   ...
   const cat=resolvedFinalCategory(r);if(cat==='Ek Bagiya Maa Ke Naam'||cat==='Ek Bagiya'){q.hasEk=true;q.ekWorks++}
 }
 const em=new Map();
...
function renderCategorySubEngineer(){
 ...
 const zeroBooked=data.filter(r=>num(r.totalBooked)<=0),zeroMandays=data.filter(r=>num(r.julyMandays)<=0),lowExp=data.filter(r=>num(r.totalSanction)>0&&r.expPct<25);
 const affected=data.filter(r=>num(r.totalBooked)<=0||num(r.julyMandays)<=0||(num(r.totalSanction)>0&&r.expPct<25));
 const alert=$('reportDeficiencyAlert');if(alert){const examples=affected.slice(0,10).map(r=>`<span class="deficiency-location"><b>${clean(r.janpad)||'—'} • ${clean(r.engineer)||'—'}</b><small>${clean(r.category)||'Other'} — ${num(r.totalBooked)<=0?'Booked ₹ 0; ':''}${num(r.julyMandays)<=0?'01 Jul–Today Mandays 0; ':''}${r.expPct<25?'व्यय '+r.expPct.toFixed(1)+'%':''}</small></span>`).join('');alert.hidden=!affected.length;alert.innerHTML=affected.length?`<div class="deficiency-summary"><strong>⚠ कमी Alert — कहाँ सुधार आवश्यक है</strong><span>Booked राशि शून्य: <b>${fmt(zeroBooked.length)}</b></span><span>01 Jul–Today Mandays शून्य: <b>${fmt(zeroMandays.length)}</b></span><span>व्यय 25% से कम: <b>${fmt(lowExp.length)}</b></span></div><div class="deficiency-locations">${examples}${affected.length>10?`<span class="deficiency-more">और ${fmt(affected.length-10)} पंक्तियाँ तालिका में लाल/पीले रंग से चिन्हित हैं।</span>`:''}</div>`:''}
 ...
 for(const r of data){const rowClass=num(r.totalBooked)<=0&&num(r.julyMandays)<=0?'deficiency-critical':(num(r.totalBooked)<=0||num(r.julyMandays)<=0||r.expPct<25?'deficiency-warning':'');h+=`<tr class="${rowClass}">${cell(r.category)}${cell(districtOf(r.janpad))}${cell(r.janpad)}${cell(r.engineer)}${cell(r.cluster)}${cell(r.workCount,true)}${cell(r.nregaAprJunMandays,true)}<td class="${num(r.julyMandays)<=0?'deficiency-cell':''}">${fmt(r.julyMandays)}</td><td class="bucket-zero">${fmt(r.b0)}</td>${cell(r.b25,true)}${cell(r.b60,true)}${cell(r.b75,true)}${cell(r.b90,true)}<td class="bucket-good">${fmt(r.b90p)}</td><td>${moneyLakh(r.totalSanction)}</td><td class="${num(r.totalBooked)<=0?'deficiency-cell':''}">${moneyLakh(r.totalBooked)}</td><td>${moneyLakh(r.remaining)}</td><td><span class="exp-pct-chip ${r.expPct===0?'critical':r.expPct<=25?'low':r.expPct>90?'good':''}">${r.expPct.toFixed(1)}%</span></td></tr>`}
 const t={};['workCount','nregaAprJunMandays','julyMandays','b0','b25','b60','b75','b90','b90p','totalSanction','totalBooked'].forEach(k=>t[k]=sum(data,k));t.remaining=Math.max(0,t.totalSanction-t.totalBooked);const ep=t.totalSanction?t.totalBooked*100/t.totalSanction:0;h+=`<tr class="total-row"><td>TOTAL</td><td></td><td></td><td></td><td></td>${cell(t.workCount,true)}${cell(t.nregaAprJunMandays,true)}${cell(t.julyMandays,true)}${cell(t.b0,true)}${cell(t.b25,true)}${cell(t.b60,true)}${cell(t.b75,true)}${cell(t.b90,true)}${cell(t.b90p,true)}<td>${moneyLakh(t.totalSanction)}</td><td>${moneyLakh(t.totalBooked)}</td><td>${moneyLakh(t.remaining)}</td><td>${ep.toFixed(1)}%</td></tr>`;if(!data.length)h+=`<tr><td colspan="18" class="empty-table">Current filter में Category × Sub Engineer data नहीं मिला।</td></tr>`;h+='</tbody>';const table=$('reportTable');table.innerHTML=h;table.classList.add('category-subeng-grid');table.style.setProperty('min-width','1740px','important');table.style.setProperty('table-layout','fixed','important');const widths=[140,72,92,135,112,54,92,92,55,55,55,55,55,55,90,90,90,66];table.querySelectorAll('tr').forEach(tr=>[...tr.children].forEach((el,i)=>{el.style.setProperty('font-size','20px','important');el.style.setProperty('line-height','1.15','important');el.style.setProperty('padding','4px 3px','important');el.style.setProperty('white-space','normal','important');if(widths[i]){el.style.setProperty('width',widths[i]+'px','important');el.style.setProperty('max-width',widths[i]+'px','important')}}));
}
...
function setPortalJanpad(j,scroll=true){
  ...
  if($('workStatusFilter'))$('workStatusFilter').value='ALL';
...
['districtFilter','janpadFilter','engineerFilter','clusterFilter','categoryFilter','workStatusFilter'].forEach(id=>$(id).addEventListener('change',()=>{
  if(id==='workStatusFilter'&&view!=='categoryworks'){document.querySelector('.tab[data-view="categoryworks"]')?.click();return}
  refreshFilters();
  if(id==='janpadFilter'){
