...
function categoryFiltered(){const d=$('districtFilter').value,j=$('janpadFilter').value,e=$('engineerFilter').value,c=$('clusterFilter').value;return categorymix.filter(r=>(d==='ALL'||districtOf(r.janpad)===d)&&(j==='ALL'||r.janpad===j)&&(e==='ALL'||r.engineer===e)&&(c==='ALL'||r.cluster===c))}
...
function ekBagiyaFiltered(){
  ...
  return ongoingDetails.filter(r=>{
    ...
    if(finalWorkCategory(r.name,r.type,r.fy)!=='Ek Bagiya')return false;
    const code=clean(r.code); if(code&&seen.has(code))return false; if(code)seen.add(code);
    return true;
  });
}
function renderEkBagiyaDashboard(){
  const src=ekBagiyaFiltered();
  const m=new Map();
  const bucket=p=>p<=0?'b0':p<=25?'b25':p<=60?'b60':p<=75?'b75':p<=90?'b90':'b90p';
  for(const r of src){
    const key=[r.janpad,r.engineer,r.cluster].map(clean).join('¦');
    if(!m.has(key))m.set(key,{district:districtOf(r.janpad),janpad:r.janpad,engineer:r.engineer,cluster:r.cluster,workCount:0,activeWorks:0,nilMandays:0,currentFYMandays:0,totalSanction:0,totalBooked:0,b0:0,b25:0,b60:0,b75:0,b90:0,b90p:0});
    const x=m.get(key),ep=num(r.expPct);x.workCount++;x.currentFYMandays+=num(r.currentFYMandays);if(num(r.currentFYMandays)>0)x.activeWorks++;else x.nilMandays++;x.totalSanction+=num(r.sanction);x.totalBooked+=num(r.booked);x[bucket(ep)]++;
  }
  let data=[...m.values()].map(r=>({...r,remaining:Math.max(0,r.totalSanction-r.totalBooked),expPct:r.totalSanction?r.totalBooked*100/r.totalSanction:0,ongoing:num(r.workCount),ekOngoing:num(r.workCount),labour:num(r.currentFYMandays)}));
  ...
  const totalWorks=src.length,totalMandays=sum(src,'currentFYMandays'),active=src.filter(r=>num(r.currentFYMandays)>0).length,nil=totalWorks-active,sanc=sum(src,'sanction'),book=sum(src,'booked'),rem=Math.max(0,sanc-book),ep=sanc?book*100/sanc:0;
  $('viewMeta').textContent=`Strict FY 2025-26 & 2026-27 title rule • ${fmt(totalWorks)} ongoing works • Current FY active ${fmt(active)} • NIL mandays ${fmt(nil)} • ${todayDate()}`;
  let h=`<thead><tr><th>District</th><th>Janpad</th><th>Sub Engineer / Upyantri</th><th>Cluster</th><th>Ek Bagiya Ongoing</th><th>Current FY Active</th><th>NIL Current FY Mandays</th><th>Current FY Mandays</th><th>0%</th><th>1–25%</th><th>26–60%</th><th>61–75%</th><th>76–90%</th><th>&gt;90%</th><th>Sanction ₹ Lakh</th><th>Booked ₹ Lakh</th><th>Remaining ₹ Lakh</th><th>Exp %</th></tr></thead><tbody>`;
  for(const r of data){h+=`<tr>${cell(r.district)}${cell(r.janpad)}${cell(r.engineer)}${cell(r.cluster)}${cell(r.workCount,true)}${cell(r.activeWorks,true)}${cell(r.nilMandays,true)}${cell(r.currentFYMandays,true)}${cell(r.b0,true)}${cell(r.b25,true)}${cell(r.b60,true)}${cell(r.b75,true)}${cell(r.b90,true)}${cell(r.b90p,true)}<td>${moneyLakh(r.totalSanction)}</td><td>${moneyLakh(r.totalBooked)}</td><td>${moneyLakh(r.remaining)}</td><td>${r.expPct.toFixed(1)}%</td></tr>`}
  const ks=['workCount','activeWorks','nilMandays','currentFYMandays','b0','b25','b60','b75','b90','b90p','totalSanction','totalBooked','remaining'],t={};ks.forEach(k=>t[k]=sum(data,k));
  h+=`<tr class="total-row"><td>TOTAL</td><td></td><td></td><td></td>${cell(t.workCount,true)}${cell(t.activeWorks,true)}${cell(t.nilMandays,true)}${cell(t.currentFYMandays,true)}${cell(t.b0,true)}${cell(t.b25,true)}${cell(t.b60,true)}${cell(t.b75,true)}${cell(t.b90,true)}${cell(t.b90p,true)}<td>${moneyLakh(t.totalSanction)}</td><td>${moneyLakh(t.totalBooked)}</td><td>${moneyLakh(t.remaining)}</td><td>${ep.toFixed(1)}%</td></tr>`;
  if(!data.length)h+=`<tr><td colspan="18" class="empty-table">Current filter में strict Ek Bagiya ongoing work नहीं मिला।</td></tr>`;
  h+='</tbody>';$('reportTable').innerHTML=h;
}
...
function renderOngoingDetails(){
 ...
 let h=`<thead><tr><th>S.No.</th><th>Janpad</th><th>Engineer / Upyantri</th><th>Cluster</th><th>GP</th><th>FY</th><th>Work Code</th><th>Work Name</th><th>Work Type</th><th>Sanction ₹</th><th>Booked ₹</th><th>Exp %</th><th>Total Mandays</th><th>Current FY Mandays</th></tr></thead><tbody>`;
 for(let i=0;i<data.length;i++){const r=data[i];h+=`<tr>${cell(i+1,true)}${cell(r.janpad)}${cell(r.engineer)}${cell(r.cluster)}${cell(r.panchayat)}${cell(r.fy)}${cell(r.code)}${cell(r.name)}${cell(r.type)}<td>${num(r.sanction).toLocaleString('en-IN',{maximumFractionDigits:2})}</td><td>${num(r.booked).toLocaleString('en-IN',{maximumFractionDigits:2})}</td><td>${num(r.expPct).toFixed(1)}%</td>${cell(r.mandays,true)}${cell(r.currentFYMandays,true)}</tr>`}
 if(!data.length)h+=`<tr><td colspan="14" class="empty-table">Current filter में Ongoing Work नहीं मिला।</td></tr>`;h+='</tbody>';$('reportTable').innerHTML=h;
}
...
function renderOfficial(){const raw=officialFiltered().map(r=>({...r,ongoing:num(r.ongoingAll),worksMR:num(r.mrAll),labour:num(r.labourAll),gpsProgress:num(r.musterGP)}));const maiharOrder=['AMARPATAN','MAIHAR','RAMNAGAR'],satnaOrder=['MAJHGAWAN','NAGOD','RAMPUR BAGHELAN','SATNA','UNCHAHARA'];const byJan=new Map(raw.map(r=>[normJanpad(r.janpad),r]));const maiharRows=maiharOrder.map(j=>byJan.get(j)).filter(Boolean);const satnaRows=satnaOrder.map(j=>byJan.get(j)).filter(Boolean);const data=[...maiharRows,...satnaRows];lastExport=data;$('viewTitle').textContent='Official Janpad Daily Report';$('viewMeta').textContent=`${data.length} Janpad • ${todayDate()}`;const keys=['totalGP','musterGP','dysfunctionalGP','labourAll','mrAll','mrs','ongoingAll','labourIndividual','mrIndividual','labourCommunity','mrCommunity','pmayLabour','pmayOngoing','pmayMR','ekLabour','ekOngoing','ekMR'];function rowHtml(r){return `<tr>${cell(districtOf(r.janpad))}${cell(r.janpad)}${cell(r.totalGP,true)}${cell(r.musterGP,true)}${cell(r.dysfunctionalGP,true)}${cell(r.labourAll,true)}${cell(r.mrAll,true)}${cell(r.ongoingAll,true)}${cell(r.mrs||0,true)}<td>${badge(r.mrAll,r.ongoingAll)}</td>${cell(r.labourIndividual,true)}${cell(r.mrIndividual,true)}${cell(r.labourCommunity,true)}${cell(r.mrCommunity,true)}<td>${badge(r.mrCommunity,r.mrAll)}</td>${cell(r.pmayLabour,true)}${cell(r.pmayOngoing,true)}${cell(r.pmayMR,true)}<td>${badge(r.pmayMR,r.pmayOngoing)}</td>${cell(r.ekLabour,true)}${cell(r.ekOngoing,true)}${cell(r.ekMR,true)}<td>${badge(r.ekMR,r.ekOngoing)}</td></tr>`}function subtotalHtml(label,rows){const t={};keys.forEach(k=>t[k]=sum(rows,k));return `<tr class="subtotal-row"><td colspan="2">${label} TOTAL</td>${cell(t.totalGP,true)}${cell(t.musterGP,true)}${cell(t.dysfunctionalGP,true)}${cell(t.labourAll,true)}${cell(t.mrAll,true)}${cell(t.ongoingAll,true)}${cell(t.mrs,true)}<td>${pct(t.mrAll,t.ongoingAll).toFixed(1)}%</td>${cell(t.labourIndividual,true)}${cell(t.mrIndividual,true)}${cell(t.labourCommunity,true)}${cell(t.mrCommunity,true)}<td>${pct(t.mrCommunity,t.mrAll).toFixed(1)}%</td>${cell(t.pmayLabour,true)}${cell(t.pmayOngoing,true)}${cell(t.pmayMR,true)}<td>${pct(t.pmayMR,t.pmayOngoing).toFixed(1)}%</td>${cell(t.ekLabour,true)}${cell(t.ekOngoing,true)}${cell(t.ekMR,true)}<td>${pct(t.ekMR,t.ekOngoing).toFixed(1)}%</td></tr>`}let h=`<thead><tr><th rowspan="2">District</th><th rowspan="2">Janpad</th><th colspan="3">Gram Panchayat</th><th colspan="5">All Types of Works / Screen-2</th><th colspan="2">Individual Land (Cat-IV)</th><th colspan="3">Community Works</th><th colspan="4">PMAY-G</th><th colspan="4">Ek Bagiya</th></tr><tr><th>Total GP</th><th>GP Progress</th><th>Dysfunctional</th><th>Labour</th><th>Works with MR</th><th>Total Ongoing Work</th><th>Muster Rolls</th><th>MR %</th><th>Labour</th><th>Works MR</th><th>Labour</th><th>Works MR</th><th>Share %</th><th>Labour</th><th>Ongoing</th><th>MR Issued</th><th>MR %</th><th>Labour</th><th>Ongoing</th><th>MR Issued</th><th>MR %</th></tr></thead><tbody>`;for(const r of maiharRows)h+=rowHtml(r);if(maiharRows.length)h+=subtotalHtml('MAIHAR',maiharRows);for(const r of satnaRows)h+=rowHtml(r);if(satnaRows.length)h+=subtotalHtml('SATNA',satnaRows);const t={};keys.forEach(k=>t[k]=sum(data,k));h+=`<tr class="total-row"><td colspan="2">GRAND TOTAL</td>${cell(t.totalGP,true)}${cell(t.musterGP,true)}${cell(t.dysfunctionalGP,true)}${cell(t.labourAll,true)}${cell(t.mrAll,true)}${cell(t.ongoingAll,true)}${cell(t.mrs,true)}<td>${pct(t.mrAll,t.ongoingAll).toFixed(1)}%</td>${cell(t.labourIndividual,true)}${cell(t.mrIndividual,true)}${cell(t.labourCommunity,true)}${cell(t.mrCommunity,true)}<td>${pct(t.mrCommunity,t.mrAll).toFixed(1)}%</td>${cell(t.pmayLabour,true)}${cell(t.pmayOngoing,true)}${cell(t.pmayMR,true)}<td>${pct(t.pmayMR,t.pmayOngoing).toFixed(1)}%</td>${cell(t.ekLabour,true)}${cell(t.ekOngoing,true)}${cell(t.ekMR,true)}<td>${pct(t.ekMR,t.ekOngoing).toFixed(1)}%</td></tr></tbody>`;$('reportTable').innerHTML=h}
function renderEngineerOfficial(){const data=sortRows(engineerOfficialData(filteredRows()).map(r=>({...r,ongoing:num(r.ongoingAll),worksMR:num(r.mrAll),labour:num(r.labourAll),gpsProgress:num(r.musterGP),ekOngoing:num(r.ekOngoing)})),'ongoing',['engineer','janpad']);lastExport=data;$('viewTitle').textContent='Sub Engineer Daily Report — Janpad Report Same Format';$('viewMeta').textContent=`${data.length} Sub Engineer rows • Official Janpad totals reconciled • ${todayDate()}`;let h=`<thead><tr><th rowspan="2">District</th><th rowspan="2">Janpad</th><th rowspan="2">Sub Engineer</th><th rowspan="2">Cluster(s)</th><th colspan="3">Gram Panchayat</th><th colspan="5">All Types of Works / Screen-2</th><th colspan="2">Individual Land (Cat-IV)</th><th colspan="3">Community Works</th><th colspan="4">PMAY-G</th><th colspan="4">Ek Bagiya</th></tr><tr><th>Total GP</th><th>GP Progress</th><th>Dysfunctional</th><th>Labour</th><th>Works with MR</th><th>Total Ongoing Work</th><th>Muster Rolls</th><th>MR %</th><th>Labour</th><th>Works MR</th><th>Labour</th><th>Works MR</th><th>Share %</th><th>Labour</th><th>Ongoing</th><th>MR Issued</th><th>MR %</th><th>Labour</th><th>Ongoing</th><th>MR Issued</th><th>MR %</th></tr></thead><tbody>`;for(const r of data){h+=`<tr>${cell(r.district)}${cell(r.janpad)}${cell(r.engineer)}${cell(r.cluster)}${cell(r.totalGP,true)}${cell(r.musterGP,true)}${cell(r.dysfunctionalGP,true)}${cell(r.labourAll,true)}${cell(r.mrAll,true)}${cell(r.ongoingAll,true)}${cell(r.mrs||0,true)}<td>${badge(r.mrAll,r.ongoingAll)}</td>${cell(r.labourIndividual,true)}${cell(r.mrIndividual,true)}${cell(r.labourCommunity,true)}${cell(r.mrCommunity,true)}<td>${badge(r.mrCommunity,r.mrAll)}</td>${cell(r.pmayLabour,true)}${cell(r.pmayOngoing,true)}${cell(r.pmayMR,true)}<td>${badge(r.pmayMR,r.pmayOngoing)}</td>${cell(r.ekLabour,true)}${cell(r.ekOngoing,true)}${cell(r.ekMR,true)}<td>${badge(r.ekMR,r.ekOngoing)}</td></tr>`}const keys=['totalGP','musterGP','dysfunctionalGP','labourAll','mrAll','mrs','ongoingAll','labourIndividual','mrIndividual','labourCommunity','mrCommunity','pmayLabour','pmayOngoing','pmayMR','ekLabour','ekOngoing','ekMR'],t={};keys.forEach(k=>t[k]=sum(data,k));h+=`<tr class="total-row"><td>TOTAL</td><td></td><td></td><td></td>${cell(t.totalGP,true)}${cell(t.musterGP,true)}${cell(t.dysfunctionalGP,true)}${cell(t.labourAll,true)}${cell(t.mrAll,true)}${cell(t.ongoingAll,true)}${cell(t.mrs,true)}<td>${pct(t.mrAll,t.ongoingAll).toFixed(1)}%</td>${cell(t.labourIndividual,true)}${cell(t.mrIndividual,true)}${cell(t.labourCommunity,true)}${cell(t.mrCommunity,true)}<td>${pct(t.mrCommunity,t.mrAll).toFixed(1)}%</td>${cell(t.pmayLabour,true)}${cell(t.pmayOngoing,true)}${cell(t.pmayMR,true)}<td>${pct(t.pmayMR,t.pmayOngoing).toFixed(1)}%</td>${cell(t.ekLabour,true)}${cell(t.ekOngoing,true)}${cell(t.ekMR,true)}<td>${pct(t.ekMR,t.ekOngoing).toFixed(1)}%</td></tr></tbody>`;$('reportTable').innerHTML=h}
