...
echo [2/9] Updating official data...
echo Using: %UPDATER%
%PYTHON_CMD% "%UPDATER%"
if errorlevel 1 (
    echo WARNING: Official data fetch failed. Existing valid data will be preserved.
...
if "%DATA_FETCH_FAILED%"=="1" (
    echo Data fetch failed - skipping data rebuild to preserve existing dashboard data.
    goto AFTER_BUILD
)
...
:AFTER_BUILD
REM Permanent Official Janpad layout is reapplied on every scheduled update.
if exist "APPLY_OFFICIAL_LAYOUT_PRINT.py" (
    echo Applying permanent Official Janpad layout...
    %PYTHON_CMD% "APPLY_OFFICIAL_LAYOUT_PRINT.py"
    if errorlevel 1 goto :BUILD_ERROR
    echo Official Janpad layout: OK
)
...
if not exist ".git" (
    echo Git repository not detected. Local update complete.
    goto :DONE
)
where git >nul 2>nul
if errorlevel 1 (
    echo Git command not found. Local update complete.
    goto :DONE
)
...
:BUILD_ERROR
echo.
echo ERROR: Dashboard rebuild/layout script failed.
pause
exit /b 1
