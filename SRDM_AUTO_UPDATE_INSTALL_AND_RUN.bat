...
if not exist "data\official-summary.csv" (
  ...
  set "FOUND_COMMIT="
  for /f "delims=" %%C in ('git rev-list --all -- "data/official-summary.csv" 2^>nul') do if not defined FOUND_COMMIT set "FOUND_COMMIT=%%C"
  if defined FOUND_COMMIT (
    git show !FOUND_COMMIT!:data/official-summary.csv >"data\official-summary.csv" 2>>"%LOG_FILE%"
    if errorlevel 1 (
      del /q "data\official-summary.csv" >nul 2>&1
    ...
    call :log ERROR: official-summary.csv is absent from disk and all Git history.
