(function(){
  ...
  function refreshClusterFilter(){
    const old=clusterFilter.value;
    const clusters=[...new Set(report.gpRows.filter(r=>(blockFilter.value==='ALL'||r.block===blockFilter.value)&&(engineerFilter.value==='ALL'||r.engineer===engineerFilter.value)).map(r=>r.cluster||'Unmapped'))].sort((a,b)=>a.localeCompare(b,'hi'));
    clusterFilter.innerHTML='<option value="ALL">— सभी क्लस्टर —</option>'+clusters.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('');
    if(clusters.includes(old))clusterFilter.value=old;
  }
  function renderCards(){
    const rows=report.gpRows.filter(r=>(blockFilter.value==='ALL'||r.block===blockFilter.value)&&(engineerFilter.value==='ALL'||r.engineer===engineerFilter.value)&&(clusterFilter.value==='ALL'||(r.cluster||'Unmapped')===clusterFilter.value));
    ...
    cards=cards.filter(x=>(statusFilter.value==='ALL'||x.status===statusFilter.value)&&(issueFilter.value==='ALL'||(issueFilter.value==='zeroIssued'?x.issued===0:x[issueFilter.value]>0))).sort((a,b)=>({poor:0,watch:1,good:2}[a.status]-{poor:0,watch:1,good:2}[b.status])||b.pendingEmb-a.pendingEmb||a.block.localeCompare(b.block)||a.engineer.localeCompare(b.engineer,'hi'));
  ...
  blockFilter.addEventListener('change',()=>setTimeout(()=>{refreshClusterFilter();renderCards()},0));
  engineerFilter.addEventListener('change',()=>{refreshClusterFilter();renderCards()});
  clusterFilter.addEventListener('change',renderCards);statusFilter.addEventListener('change',renderCards);issueFilter.addEventListener('change',renderCards);
  resetButton.addEventListener('click',()=>{clusterFilter.value='ALL';statusFilter.value='ALL';issueFilter.value='ALL';renderCards()});
  refreshClusterFilter();renderCards();
})();
